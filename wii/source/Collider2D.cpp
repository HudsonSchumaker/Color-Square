...
Collider2D::Collider2D(Point2D p) : x(p.getX()), y(p.getY()), w(0), h(0) {}
...
Point2D Collider2D::getLocation() {
    return Point2D(this->x, this->y);
}

Dimension2D Collider2D::getSize() {
    return Dimension2D(this->w, this->h);
}

bool Collider2D::contains(Point2D p) {
